...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void _generateQRCode() {
    ...
    Future<String> _getBuyerAddress() async {
      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      if (authProvider.user != null) {
        // If you have UserModel in AuthProvider
        if (authProvider.userModel != null) {
          return authProvider.userModel!.fullAddress;
        }

        // If you need to fetch from UserProvider
        final userProvider = Provider.of<UserProvider>(context, listen: false);
        final userModel = await userProvider.getUser(authProvider.user!.uid);
        return userModel?.fullAddress ?? 'Address not provided';
      }
  ...
  String _getSellerAddress() {
    return sellerInfo?.fullAddress ?? 'Seller address not available';
  }
  ...
  Future<void> _confirmPaymentSuccess() async {
    try {
      ...
      if (authProvider.user == null) {
        ...
        final userProvider = Provider.of<UserProvider>(context, listen: false);
        final buyerModel = await userProvider.getUser(authProvider.user!.uid);

      // Step 1: Update offer status to paid
      ...
      // Step 2: Create Order with proper address
      final newOrder = OrderModel(
        id: '', // Will be generated by OrderProvider
        productId: widget.offer.productId,
        productTitle: widget.offer.productTitle,
        productImageUrl: widget.offer.productImageUrl,
        productPrice: widget.offer.offerAmount,
        buyerId: authProvider.user!.uid,
        buyerName: authProvider.user!.displayName ?? 'Unknown',
        buyerPhone: authProvider.user!.phoneNumber ?? '',
        sellerId: widget.offer.sellerId,
        sellerName: widget.offer.sellerName,
        sellerPhone: widget.offer.sellerPhone,
        status: selectedPaymentMethod == 'upi' ? OrderStatus.confirmed : OrderStatus.pending,
        shippingMethod: ShippingMethod.courier,
        deliveryAddress:  buyerModel!.address, // Use the fetched address
        createdAt: DateTime.now(),
        updatedAt: DateTime.now(),
        isPaid: selectedPaymentMethod == 'upi',
        paymentMethod: selectedPaymentMethod == 'upi' ? 'UPI Payment' : 'Cash on Delivery',
        notes: 'Order created from accepted offer: ${widget.offer.id}',
      );
      ...
      if (!success) {
        throw Exception(orderProvider.errorMessage ?? 'Failed to create order');
      }

      final createdOrder = orderProvider.orders.firstWhere(
            (order) => order.productId == widget.offer.productId &&
            order.buyerId == authProvider.user!.uid &&
            order.createdAt.isAfter(DateTime.now().subtract(const Duration(minutes: 1))),
      );

      if (mounted) {
        _showPaymentSuccessDialog(createdOrder);
      }
    } catch (e) {
  ...
  void _showPaymentSuccessDialog(OrderModel createdOrder) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text('The seller will be notified and will proceed with shipping.'),
            const SizedBox(height: 8),
            Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(
